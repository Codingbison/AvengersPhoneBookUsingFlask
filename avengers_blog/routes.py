from avengers_blog import app, db, Message, mail
from flask import render_template, request, redirect, url_for

# Import for Forms
from avengers_blog.forms import UserInfoForm, BlogPostForm, LoginForm

# Import for Models
from avengers_blog.models import User, Post, check_password_hash
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        db.session.add(user)
        # Save into to database
        db.session.commit()

        # Flask Email Sender
        msg = Message(f'Thanks for signing up, {username}!', recipients=[email])
        msg.body = {'Congrats on signing up! Looking forward for your posts!'}
